import { RemoveMovieFromAnimeUseCase } from '../../../../../src/application/use-cases/anime/remove-movie-from-anime.use-case.js';
import { IAnimeRepository } from '../../../../../src/domain/repositories/anime.repository.js';
import { NotFoundError } from '../../../../../src/application/errors/index.js';
import Anime from '../../../../../src/domain/entities/Anime.entity.js';
import Category from '../../../../../src/domain/entities/Category.entity.js';
import Genre from '../../../../../src/domain/entities/Genre.entity.js';
import Movie from '../../../../../src/domain/value-objects/movie.value-object.js';
import Season from '../../../../../src/domain/value-objects/season.value-object.js';

describe('RemoveMovieFromAnimeUseCase', () => {
  let useCase: RemoveMovieFromAnimeUseCase;
  let animeRepository: jest.Mocked<IAnimeRepository>;

  const mockCategory = Category.create('Shounen', 'Shounen', 'Teens', 'Action anime for boys');
  const mockGenre = Genre.create('Action', 'Action genre', false);

  const createMockAnimeWithMovie = () => {
    const movie = Movie.create({
      name: 'Naruto: The Movie',
      releaseDate: new Date('2004-08-21'),
    });
    const movie2 = Movie.create({
      name: 'Naruto: Second Movie',
      releaseDate: new Date('2005-08-21'),
    });
    const season = Season.create({
      seasonNumber: 1,
      releaseDate: new Date('2002-10-03'),
      totalEpisodes: 220,
    });
    const anime = Anime.create({
      imageUrl: 'https://example.com/naruto.jpg',
      name: 'Naruto',
      synopsis: 'A ninja anime',
      category: mockCategory,
      genres: [mockGenre],
      animeType: 'mixed',
      productionType: 'original',
      typeOfMaterialOrigin: 'none',
      movies: [movie, movie2],
      seasons: [season],
      isAdultContent: false,
    });
    return anime;
  };

  beforeEach(() => {
    animeRepository = {
      findById: jest.fn(),
      findByTitle: jest.fn(),
      findAll: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
      delete: jest.fn(),
    };

    useCase = new RemoveMovieFromAnimeUseCase(animeRepository);
  });

  it('deve remover um filme do anime com sucesso', async () => {
    const anime = createMockAnimeWithMovie();
    const removeMovieData = {
      animeId: anime.id.value,
      name: 'Naruto: The Movie',
      releaseDate: new Date('2004-08-21'),
    };

    animeRepository.findById.mockResolvedValue(anime);
    animeRepository.update.mockImplementation(async (a) => a);

    const result = await useCase.execute(removeMovieData);

    expect(animeRepository.findById).toHaveBeenCalledWith(removeMovieData.animeId);
    expect(animeRepository.update).toHaveBeenCalled();
    expect(result.movies).toHaveLength(1);
  });

  it('deve lançar NotFoundError quando o anime não existir', async () => {
    const removeMovieData = {
      animeId: 'non-existent-id',
      name: 'Some Movie',
      releaseDate: new Date('2024-01-01'),
    };

    animeRepository.findById.mockResolvedValue(null);

    await expect(useCase.execute(removeMovieData)).rejects.toThrow(NotFoundError);
    expect(animeRepository.update).not.toHaveBeenCalled();
  });
});
